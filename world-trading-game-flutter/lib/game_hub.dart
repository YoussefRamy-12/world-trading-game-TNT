...
class _MarketplacePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('Marketplace'), actions: [IconButton(onPressed: onChanged, icon: const Icon(Icons.refresh))]), body: ListView(padding: const EdgeInsets.all(20), children: [
      ...
      if (open.isEmpty) const Card(child: ListTile(title: Text('No countries are currently listed.'), subtitle: Text('Owned countries can be listed from the country detail screen.'))),
...
class _LeaderboardPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('Leaderboard')), body: ListView(padding: const EdgeInsets.all(20), children: [
      const Card(child: Padding(padding: EdgeInsets.all(16), child: Text('Ranking is calculated from the existing game net-worth function.'))), const SizedBox(height: 12),
      for (var i = 0; i < rows.length; i++) _rankTile(context, i + 1, rows[i]),
      if (rows.isEmpty) const Card(child: ListTile(title: Text('Leaderboard is not available yet.'))),
    ]));
  }
  Widget _rankTile(BuildContext context, int rank, Map<String, dynamic> row) {
    final id = row['player_id']?.toString(); final mine = id == player?.id; final name = (row['display_name'] ?? row['player_name'] ?? 'Player $rank').toString(); final worth = _number(row['net_worth']);
    return Card(child: ListTile(leading: CircleAvatar(child: Text('$rank')), title: Text(name, style: TextStyle(fontWeight: mine ? FontWeight.bold : FontWeight.normal)), subtitle: mine ? const Text('You') : null, trailing: Text('\$$worth', style: const TextStyle(fontWeight: FontWeight.bold))));
  }
}
...
class _Stat extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), border: Border.all(color: Theme.of(context).colorScheme.outlineVariant)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon), const SizedBox(height: 8), Text(title), const SizedBox(height: 3), Text(value, style: const TextStyle(fontWeight: FontWeight.bold))]));
}
