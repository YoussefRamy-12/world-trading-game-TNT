...
class _GameHubPageState extends State<GameHubPage> {
  ...
  Widget build(BuildContext context) {
    final pages = [
      RealWorldMapPage(game: widget.game),
...
class _PortfolioPage extends StatelessWidget {
  final Player? player; final List<Country> countries; final List<Map<String, dynamic>> netWorths;
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('My Portfolio')), body: ListView(padding: const EdgeInsets.all(20), children: [
      Card(child: Padding(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(player?.displayName.isNotEmpty == true ? player!.displayName : 'Player', style: Theme.of(context).textTheme.headlineSmall), const SizedBox(height: 16), Row(children: [Expanded(child: _Stat(title: 'Countries', value: '${owned.length}', icon: Icons.public)), const SizedBox(width: 12), Expanded(child: _Stat(title: 'Country value', value: '\$$countryValue', icon: Icons.trending_up)), const SizedBox(width: 12), Expanded(child: _Stat(title: 'Net worth', value: netWorth == null ? '—' : '\$$netWorth', icon: Icons.account_balance_wallet))])]))) ,
      const SizedBox(height: 12), Text('My Countries', style: Theme.of(context).textTheme.titleLarge), const SizedBox(height: 8),
...
class _MarketplacePage extends StatelessWidget {
  final Player? player; final List<Country> countries; final List<CountryListing> listings; final GameRepository repository; final ActionGuard guard; final Future<void> Function() onChanged;
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('Marketplace'), actions: [IconButton(onPressed: onChanged, icon: const Icon(Icons.refresh))]), body: ListView(padding: const EdgeInsets.all(20), children: [
      const Card(child: Padding(padding: EdgeInsets.all(16), child: Text('Buy countries from other players. Bidding uses the existing marketplace RPCs.'))), const SizedBox(height: 12),
...
class _LeaderboardPage extends StatelessWidget {
  final Player? player; final List<Map<String, dynamic>> netWorths;
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('Leaderboard')), body: ListView(padding: const EdgeInsets.all(20), children: [for (var i = 0; i < rows.length; i++) _rankTile(i + 1, rows[i]), if (rows.isEmpty) const Card(child: ListTile(title: Text('Leaderboard is not available yet.')))]));
  }
  Widget _rankTile(int rank, Map<String, dynamic> row) { final mine = row['player_id']?.toString() == player?.id; final name = (row['display_name'] ?? row['player_name'] ?? 'Player $rank').toString(); return Card(child: ListTile(leading: CircleAvatar(child: Text('$rank')), title: Text(name, style: TextStyle(fontWeight: mine ? FontWeight.bold : FontWeight.normal)), subtitle: mine ? const Text('You') : null, trailing: Text('\$${_number(row['net_worth'])}'))); }
}

class _Stat extends StatelessWidget { final String title; final String value; final IconData icon; const _Stat({required this.title, required this.value, required this.icon}); @override Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), border: Border.all(color: Theme.of(context).colorScheme.outlineVariant)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon), const SizedBox(height: 8), Text(title), Text(value, style: const TextStyle(fontWeight: FontWeight.bold))])); }
