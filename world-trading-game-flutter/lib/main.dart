  ...
  Future<void> submit() async {
    ...
    try {
      if (signup) {
        ...
        if (displayName.isEmpty) throw StateError('Display name is required.');
        await auth.signUp(email: email.text, password: password.text, displayName: displayName);
      } else {
...
class _LobbyPageState extends State<LobbyPage> {
  ...
  @override
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('Game Lobby'), actions: [IconButton(onPressed: load, icon: const Icon(Icons.refresh)), IconButton(onPressed: () => AuthService().signOut(), icon: const Icon(Icons.logout))]),
